 ...
 * next year lands on this screen with no code written for it.
 */
export default function DeviceScreen() {
  const { id } = useLocalSearchParams<{ id: string }>();
  const device = useDevice(id);
  const { editable, loading } = useDevices();

  if (!device) {
    return (
      <Screen back="Devices" title="Device">
        <Card>
  ...
  return (
    <Screen back="Devices" title={device.name} subtitle={device.description}>
...
function Overview({ device }: { device: DeviceView }) {
 ...
 */
function Controls({ device }: { device: DeviceView }) {
...
function confirmDangerous(control: ControlSpec, proceed: () => void): void {
  const message =
    control.consequence ?? `${control.label} moves real hardware. Continue?`;
...
function Readings({ device }: { device: DeviceView }) {
 ...
 * being charted.
 */
function History({ device }: { device: DeviceView }) {
...
function Manage({ device }: { device: DeviceView }) {
  ...
  const forget = () => {
    ...
    const proceed = () => {
      ...
      void remove(device.id)
        .then(() => router.replace('/(tabs)/devices'))
